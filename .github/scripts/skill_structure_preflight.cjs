...
async function upsertComment(github, context, existingComment, body) {
	if (existingComment) {
...
async function runStructurePreflight({ github, context }) {
	...
	if (errors.length > 0) {
		...
		await upsertComment(github, context, existingComment, formatStructureComment(errors, managesLabel));
		...
		await upsertComment(github, context, existingComment, formatStructureComment([]));
	}
